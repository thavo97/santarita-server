...
<?php
    ...
    if("GET_ALL_PRODUCTS" == $action){
        $products_data = array();
        $sql = "select code, name, cost, price from $table order by name limit 0,50";
    ...
    if ("GET_PAGE_PRODUCTS" == $action) {
        ...
        $sql = "select code, name, cost, price from $table order by name limit $start,$end";
        $result = $conn->query($sql);
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                $products_pages[]= $row;
    ...
    if("GET_SEARCH_PRODUCTS" == $action){
        ...
        $sql = "select code, name, cost, price from $table where name like '%$character%' order by name";
        $result = $conn->query($sql);
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                $products_search[]= $row;
    ...
    if("UPDATE_PRICE" == $action){
        ...
        $sql = "update $table SET price = '$price_product',  cost = '$cost_product' WHERE code ='$code_product'";
